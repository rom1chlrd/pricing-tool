...
def fetch_news_and_sentiment(api_key, query="Middle East OR Iran OR war"):
    ...
    try:
        ...
        top_news = [{"title": a["title"], "url": a["url"]} for a in articles[:4]]
        return max(0.5, min(lam, 10.0)), top_news
    except:
        return 2.0, []
...
with col_gauge:
    fig_gauge = go.Figure(go.Indicator(
        ...
        value = dynamic_lam,
        domain = {'x': [0, 1], 'y': [0, 1]},
        title = {'text': "Sauts $\lambda$ / an", 'font': {'size': 20}},
...
with col_news:
    st.subheader("Actualités Live")
    if top_news:
        for news in top_news:
            # Affichage en liste avec liens cliquables
...
col_res1.metric("Prix Put (BS)", f"{price_bs:.3f} €")
col_res2.metric("Prix Put (MJD Live)", f"{price_mjd:.3f} €", f"{(price_mjd-price_bs):.3f} € de Surprime", delta_color="inverse")

fig_mc = go.Figure()
for i in range(50):
    fig_mc.add_trace(go.Scatter(y=paths[i, :], mode='lines', line=dict(width=1, color='rgba(255, 75, 75, 0.4)')))
fig_mc.add_hline(y=K, line_dash="dash", line_color="cyan", annotation_text="Strike K", annotation_font_color="cyan")
fig_mc.update_layout(showlegend=False, height=400, template="plotly_dark", xaxis_title="Jours", yaxis_title="Prix")
